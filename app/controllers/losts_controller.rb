class LostsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @lost.update(lost_params)
        format.html { redirect_to @lost, notice: "Lost was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to losts_path, notice: "Lost was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_lost
      @lost = Lost.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def lost_params
      params.expect(lost: [ :title, :description, :thumbnail_url, :video_url, :pro ])
    end
end
